...
# logging.basicConfig(filename=os.path.join('Logs', 'admin_panel_finder.log'),
...
def display_banner():
    ...
    banner = pyfiglet.figlet_format(" Vamp Advanced Admin Finder", font="slant")
...
class AdminPanelFinder:
    ...
    def get_user_input(self):
        """Gathers user input for the Admin Panel Finder."""
        base_url = input(Fore.WHITE + "[i] Enter target base URL (e.g., https://example.com): ").strip()
        base_url = self.clean_url(base_url)
        status(f"Target base URL set to: {base_url}")

        wordlist_file = input(Fore.WHITE + "[i] Enter paths wordlist file (default: admin_panel_paths.txt, press Enter for default): ").strip() or os.path.join(wordlists_dir, "admin_panel_paths.txt")
        ...
        print(Fore.YELLOW + Style.BRIGHT + "Starting admin panel path search...")
        return base_url, paths, num_threads

    def run(self):
        """Runs the admin panel finder with a thread pool."""
        ...
        if self.found_panels:
            success("Admin panels found:")
            for panel in self.found_panels:
                print(panel)
        else:
            status("No admin panels found.")

    def check_path(self, path):
        """Attempts to find the admin panel using the provided path."""
        ...
        try:
            ...
            if response.status_code == 200 and ("admin" in response.text.lower() or "login" in response.text.lower()):
                self.found_panels.append(full_url)
                success_msg = (
                    f"\n[{self.attempt_number}/{self.total_attempts}] Tested - {full_url} [{progress}]\n"
                    f"\n{Fore.GREEN + Style.BRIGHT}SUCCESS!{Style.RESET_ALL} Admin panel found!\n"
                    f"{Fore.YELLOW}{'-'*40}\n"
                ...
                print(success_msg)
                with open(os.path.join(results_dir, 'AdminPanel_Found.txt'), 'a') as writer:
